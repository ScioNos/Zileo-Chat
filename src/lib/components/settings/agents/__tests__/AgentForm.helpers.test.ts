...
expect(toggleSelection(original, 'CalculatorTool')).toEqual(['MemoryTool', 'TodoTool', 'CalculatorTool']);
...
expect(validateAgentForm(validationInput({ name: 'a'.repeat(65) })).name).toBe('agents_name_error');
expect(validateAgentForm(validationInput({
name: 'AgentOne',
existingAgents: [{ id: 'other', name: 'agentone' }]
})).name).toBe('agents_name_duplicate');
expect(validateAgentForm(validationInput({
name: 'AgentOne',
agentId: 'same',
...
expect(validateAgentForm(validationInput({ availableModelsCount: 0, model: '', selectedModel: null })).model)
.toBe('agents_no_models_error');
expect(validateAgentForm(validationInput({ model: '', selectedModel: null })).model)
.toBe('agents_model_required');
expect(validateAgentForm(validationInput({ selectedModel: null })).model)
.toBe('agents_model_not_found');
expect(validateAgentForm(validationInput({ maxToolIterations: 0 })).maxToolIterations)
.toBe('agents_max_iterations_error');
expect(validateAgentForm(validationInput({ maxToolIterations: 201 })).maxToolIterations)
.toBe('agents_max_iterations_error');
expect(validateAgentForm(validationInput({ systemPrompt: '   ' })).systemPrompt)
.toBe('agents_system_prompt_required');
expect(validateAgentForm(validationInput({ systemPrompt: 'a'.repeat(10001) })).systemPrompt)
